# -*- coding: utf-8 -*-
"""
Created on Thu Jul 23 10:08:44 2026
...
def main():
    ...
    with st.sidebar:
        ...
        # Read GEMINI_API_KEY automatically from Streamlit Secrets or environment
        ...
        if not gemini_api_key:
            key_input = st.text_input(
                ...
                type="password",
                help="Paste your Google Gemini API key here and click Save Key.",
            )
            if st.button("💾 Save Key"):
                cleaned_key = key_input.strip().strip('"').strip("'")
                st.session_state["saved_gemini_key"] = cleaned_key
                st.success("Gemini API Key saved!")

            gemini_api_key = st.session_state.get("saved_gemini_key", key_input.strip())
            ...
            if gemini_api_key:
                try:
                    ...
                    with st.chat_message("assistant"):
                        # Convert Streamlit chat session history to Gemini Contents format
                        contents = []
                        for msg in st.session_state.chat_messages:
                ...
                st.warning("Please provide a Gemini API Key to run the assistant.")
